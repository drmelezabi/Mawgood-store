...
export class Orders {
  ...
  async getOrders(userId: number): Promise<ordersList> {
    try {
      ...
      if (result.rows.length > 0) {
        ...
        throw new Error(`no orders for user number ${userId}`);
      }
    } catch (err) {
      throw new Error(
  ...
  async getDoneOrdersByUserId(userId: number): Promise<ordersList> {
    try {
      const sql = `SELECT orders.id as id FROM orders WHERE user_id = ${
        userId as unknown as number
      } AND status = 'complete'`;
  ...
  async createOrder(
    ...
    try {
      ...
      const sql3 = `SELECT orders.id as item_id, order_products.created_at, order_products.quantity, products.name as product_name, 
          order_products.quantity * products.price as invoice, products.id as product_id , orders.status
          FROM order_products
            INNER JOIN products ON order_products.product_id = products.id
            INNER JOIN orders ON order_products.order_id = orders.id 
      ...
      client.release();
      const list: ItemInvoice[] = result2.rows;

      return { order_id: orederID, items: list };
    ...
    try {
      ...
      if (ordersArray.length > 0) {
        ...
        for (let i = 0; i < ordersArray.length; i++) {
          ...
          const sql = `SELECT orders.id as order_id, order_products.created_at, order_products.quantity, products.name as product_name, 
          order_products.quantity * products.price as invoice, products.id as product_id , orders.status
          FROM order_products
          INNER JOIN products ON order_products.product_id = products.id
          INNER JOIN orders ON order_products.order_id = orders.id 
        ...
        throw new Error(`no orders for user number ${orderId}`);
      }
    } catch (err) {
      throw new Error(
  ...
  async deleteOrder(
    ...
    try {
      ...
      if (list.length > 0) {
        ...
        throw new Error(`there is no order :${orderId} to delete`);
      }
    } catch (err) {
      throw new Error(
